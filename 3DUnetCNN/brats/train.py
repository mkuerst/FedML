...
from unet3d.model import unet_model_3d, isensee2017_model, isensee_global_model, isensee_multiheads_model, isensee_soft_mh_model
...
def fetch_training_data_files(task):
    # FOR LOCAL MODEL
    training_data_files, config['labels'], config['n_labels'] = fetch_data.prepare_data(task[0])
    config['labels'] = (1,2)
    config['n_labels'] = len(config['labels'])
...
def main(tasks, overwrite=False):
    ...
    # task = 'global(144,144,48)_mdl1e_hs_1mod'
    ...
    # # MULTIHEADS MODEL
    # task_gl = 'global(64,64,48)_ismh_bal'
    # config["data_file"] = os.path.abspath(task_gl+".h5")
    # config["model_file"] = os.path.abspath(task_gl+"_model.h5")
    # config["training_file"] = os.path.abspath(task_gl+"_training_ids.pkl")
    # config["validation_file"] = os.path.abspath(task_gl+"_validation_ids.pkl")
    ...
    #     model, opt, loss,ldcm = isensee_soft_mh_model(input_shape=config['input_shape'],n_labels=config['n_labels'])
...
# KERAS_BACKEND=theano THEANO_FLAGS=mode=FAST_RUN,device=cpu,floatX=float32 python train.py
