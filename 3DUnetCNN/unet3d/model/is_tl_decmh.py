...
# tf.config.experimental_run_functions_eagerly(True)
def is_tl_decmh_model(input_shape=(4, 128, 128, 128), n_base_filters=16, depth=5, dropout_rate=0.3,
    ...
    encoded1 = current_layer
    encoded2 = current_layer
    encoded3 = current_layer

    output_layer12 = create_branch(encoded1,depth, level_output_layers, level_filters, 3, n_segmentation_levels)
    output_layer34 = create_branch(encoded2,depth, level_output_layers, level_filters, 3, n_segmentation_levels)
    output_layer5 = create_branch(encoded3,depth, level_output_layers, level_filters, 2, n_segmentation_levels)

    # output_layer12 = Lambda(lambda x: task_test(x,task_labels,0))
    # output_layer34 = Lambda(lambda x: task_test(x,task_labels,1))
    # output_layer5 = Lambda(lambda x: task_test(x,task_labels,2))
    ...
    loss = {'out12': get_label_dice_loss(w1,0), \
        'out34': get_label_dice_loss(w2,1), \
        'out5': get_label_dice_loss(w3,2) }
    ...
    model = Model(inputs=[inputs,w1,w2,w3], outputs=outputs) 
...
def create_branch(current_layer,depth, level_output_layers, level_filters, n_labels, n_segmentation_levels):
...
def task_test(x, task_labels, index):
    if task_labels[0][index] == 0:
        x = (x-x)+1e-16
    return x