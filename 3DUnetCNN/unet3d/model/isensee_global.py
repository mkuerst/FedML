...
def isensee_global_model(input_shape=(4, 128, 128, 128), n_base_filters=16, depth=5, dropout_rate=0.3,
    ...
    label_wise_dice_metrics = {'out12': [get_label_dice_coefficient_function(0),get_label_dice_coefficient_function(1)], \
                                'out34': [get_label_dice_coefficient_function(0),get_label_dice_coefficient_function(1)], \
                                    'out5': [get_label_dice_coefficient_function(0)]}
    #label_wise_loss = [get_label_dice_loss(0,2,weights,0), get_label_dice_loss(2,4,weights,1), get_label_dice_loss(4,5,weights,2)]
    label_wise_loss = {'out12': get_label_dice_loss(0,2,weights,0), 'out34': get_label_dice_loss(0,2,weights,1), 'out5': get_label_dice_loss(0,1,weights,2) }
    model = Model(inputs=[inputs,weights], outputs=outputs) 
    model.compile(optimizer=optimizer(lr=initial_learning_rate), loss=label_wise_loss, metrics=label_wise_dice_metrics)
