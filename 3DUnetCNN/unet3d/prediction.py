...
def multi_class_prediction(prediction, affine):
    ...
    for i in range(prediction.shape[1]):
        print("saved label prediction "+str(i))
...
def run_validation_case(data_index, output_dir, model, data_file, training_modalities,
    ...
    task_label = np.asarray([data_file.root.label[data_index]])
    ...
    if patch_shape == test_data.shape[-3:]:
        prediction = predict(model, test_data,task_label, permute=permute)
    else:
    ...
    if isinstance(prediction_image, list):
        for i, image in enumerate(prediction_image):
            image.to_filename(os.path.join(output_dir, "prediction_{0}.nii.gz".format(i ))) #i+1
    else:
...
def predict(model, data,task_label=[1,1,1], permute=False):
    if permute:
        predictions = list()
        ...
        data1 = task_label[0][0][0]*data
        data2 = task_label[0][0][1]*data
        data3 = task_label[0][0][2]*data
        
        pred = model.predict([data1,data2,data3])

        if len(pred)>1:
            conc = np.hstack((pred[0],pred[1],pred[2]))
            print(conc.shape)
            return conc
        else:
            return pred
