...
def read_image_files(image_files, image_shape=None, crop=None, label_indices=None):
    ...
    image_list = list()
    #print("image_files var: "+str(image_files)) # BEWARE CHANGED
    ...
    for index, image_file in enumerate(image_files):
        if (label_indices is None and (index + 1) == len(image_files)) \
            ...
            interpolation = "linear"     #CHANGED linear
...
def resize(image, new_shape, interpolation="linear"):
    ...
    #return nilearn.image.resample_img(image, target_affine=image.affine, target_shape=new_shape, interpolation='continuous', copy=True, order='F', clip=True, fill_value=0, force_resample=False)



def add_pad(image, new_shape):
    height, width, z = image.shape
    new_height = new_shape.shape[0]
    new_width = new_shape.shape[1]
    new_z = new_shape.shape[2]

    final_image = np.zeros((new_height, new_width,new_z))

    pad_left = int((new_width - width) / 2)
    pad_top = int((new_height - height) / 2)
    pad_bottom = int((new_z - z) / 2)
    # Replace the pixels with the image's pixels
    final_image[pad_top:pad_top + height, pad_left:pad_left + width, pad_bottom:pad_bottom+z] = image
